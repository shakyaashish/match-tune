...
public class CircleMenuView extends FrameLayout {
    ...
    private static final float DEFAULT_DISTANCE = DEFAULT_BUTTON_SIZE * 4.5f;
    private static final float DEFAULT_RING_SCALE_RATIO = 1.3f;
    private static final float DEFAULT_CLOSE_ICON_ALPHA = 0.0f;
    ...
    private void initMenu(int menuButtonColor) {
       ...
       // mMenuButton.setImageResource(mIconMenu);
        mMenuButton.setAlpha(DEFAULT_CLOSE_ICON_ALPHA);
    ...
    private void initButtons(@NonNull Context context, @NonNull List<Integer> icons, @NonNull List<Integer> colors) {
        ...
        for (int i = 0; i < buttonsCount; i++) {
            ...
            button.setScaleX(0);
            button.setScaleY(0);
            button.setLayoutParams(new LayoutParams(130, 130));
            if(i==4) {
                button.setBackgroundResource(R.drawable.tour);
            }else if(i==3) {
                button.setBackgroundResource(R.drawable.demo);
            }else if(i==2) {
                button.setBackgroundResource(R.drawable.camera);
            }else if(i==1) {
                button.setBackgroundResource(R.drawable.folder);
            }else{
    ...
    private void offsetAndScaleButtons(float centerX, float centerY, float angleStep, float offset, float scale) {
        for (int i = 0, cnt = mButtons.size(); i < cnt; i++) {
            final float angle = angleStep * i - 170; //90
            final float x = (float) Math.cos(Math.toRadians(angle)) * offset+30;
            final float y = (float) Math.sin(Math.toRadians(angle)) * offset+20;
    ...
    private Animator getButtonClickAnimation(final @NonNull Button button) {
        ...
        final float stepAngle = 180f / mButtons.size();
        final float rOStartAngle = (180 - stepAngle + stepAngle * buttonNumber);
        final float rStartAngle = rOStartAngle > 180 ? rOStartAngle % 180: rOStartAngle;
        ...
        final ObjectAnimator rotateButton = ObjectAnimator.ofFloat(button, "rotation", 0f, 0f);
...
//        }
        ...
        final ObjectAnimator ring = ObjectAnimator.ofFloat(mRingView, "angle", 180);
    ...
    private Animator getOpenMenuAnimation() {
        ...
        final float angleStep = 200f / buttonsCount;
    ...
    private Animator getCloseMenuAnimation() {
        ...
        final ObjectAnimator alpha2 = ObjectAnimator.ofFloat(mMenuButton, "alpha", 0f);
    ...
    private void openOrClose(boolean open, boolean animate) {
        ...
        if (animate) {
           ...
           // mMenuButton.setImageResource(open ? mIconClose : mIconMenu);
            mMenuButton.setAlpha(open ? DEFAULT_CLOSE_ICON_ALPHA : 0f);
